...
# i <- plot(net.precov, vertex.cex=1.5, mode="kamadakawai", vertex.col=3, edge.col="gray20")

make_D3_network(net.precov)
...
make_D3_network(emptynet, charge = 0) # charge 0 to prevent nodes from falling into one another
...
```{r essl.net, cache=FALSE, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.height=4, fig.width=4, fig.align="center"}
meandeg.essl <- 4
prop.essl <- 0.1
essl <- rep(0, n)
...
par(mai=c(0,0,0,0))
# plot(net.essl, vertex.cex=1.5, vertex.col=3essl, edge.col="gray20")
make_D3_network(net.essl,charge = -1, group = essl) # low charge to keep elements mostly together
...
make_D3_network(net.comb.1, charge = -1.5,  group = essl)
...
make_D3_network(net.comb.2, charge = -10,  group = essl)
