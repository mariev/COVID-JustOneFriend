...
make_D3_network(net.precov, charge = 0) # charge 0 to prevent nodes from falling into one another
```
...
```{r emptynet, cache=FALSE, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
par(mai=c(0,0,0,0))
...
make_D3_network(net.essl, charge = 0) # charge 0 to prevent nodes from falling into one another
```
...
make_D3_network(net.comb.1, charge = -1.5, group = essl) # charge 0 to prevent nodes from falling into one another
```
